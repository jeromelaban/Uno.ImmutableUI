...
// ******************************************************************
//
// This file a modified version of https://github.com/nventive/Uno.CodeGen/blob/cdf03c2c7bfbf901bb00f971ff91492a8fe87ffa/src/Uno.CodeGen/ImmutableGenerator.cs
// to support for the generation of IsPropertySet in the original immutable type.
//
using System;
...
namespace Uno
{
	...
	/// </remarks>
	public class TemporaryImmutableGenerator : SourceGenerator
	{
		...
		public override void Execute(SourceGeneratorContext context)
		{
			...
			_generatedImmutableAttributeSymbol = context.Compilation.GetTypeByMetadataName("Uno.GeneratedImmutableModelAttribute");
			...
			using (builder.BlockInvariant($"namespace {typeSymbol.ContainingNamespace}"))
			{
				...
				using (builder.BlockInvariant($"{typeSymbol.GetAccessibilityAsCSharpCodeString()} {abstractClause}partial class {symbolNameWithGenerics}"))
				{
					foreach (var property in properties)
					{
						...
						builder.AppendLineInvariant($"/// Determines if {defaultMemberName} has been set by the builder.");
						builder.AppendLineInvariant($"/// </summary>");
						builder.AppendLineInvariant($"public bool Is{property.property.Name}Set {{{{ get; }}}}");
					}
...
public bool Is{prop.Name}Set =>  _is{prop.Name}Set;
...
{{
	return builder.ToOptionImmutable();
					...
					using (builder.BlockInvariant($"public {symbolName}(Builder builder){baseConstructorChaining}"))
					{
						...
						{
							builder.AppendLineInvariant($"Is{prop.Name}Set = builder.Is{prop.Name}Set;");
...
{{
	...
	{{
			...
			SymbolNames symbolNames, IPropertySymbol[] typeProperties)
		{
			...
			{
				builder.AppendLineInvariant(
					$"#warning {nameof(TemporaryImmutableGenerator)}: you should add the partial modifier to the class {symbolNames.SymbolNameWithGenerics}.");
			}

			if (typeSymbol.IsValueType)
			{
				builder.AppendLineInvariant(
					$"#error {nameof(TemporaryImmutableGenerator)}: Type {symbolNames.SymbolNameWithGenerics} **MUST** be a class, not a struct.");
			}

			if (baseTypeInfo.isBaseType && baseTypeInfo.baseType == null)
			{
				builder.AppendLineInvariant(
					$"#error {nameof(TemporaryImmutableGenerator)}: Type {symbolNames.SymbolNameWithGenerics} **MUST** derive from an immutable class.");
			}
			...
			void CheckTypeImmutable(ITypeSymbol type, string typeSource, bool constraintsChecked = false)
			{
				if (type is ITypeParameterSymbol typeParameter)
				{
					...
					{
						builder.AppendLineInvariant(
							$"#error {nameof(TemporaryImmutableGenerator)}: {typeSource} is of generic type {type} which isn't restricted to immutable. You can also make your class abstract.");
					}
				...
				{
					builder.AppendLineInvariant(
						$"#error {nameof(TemporaryImmutableGenerator)}: {typeSource} type {type} IS A BUILDER! It cannot be used in an immutable entity.");
				}
				else if (!type.IsImmutable(_generationOptions.treatArrayAsImmutable))
				{
					...
					{
						builder.AppendLineInvariant(
							$"#error {nameof(TemporaryImmutableGenerator)}: {typeSource} type {type} is an array, which is not immutable. "
					...
					{
						builder.AppendLineInvariant(
							$"#error {nameof(TemporaryImmutableGenerator)}: {typeSource} type {type} is not immutable. It cannot be used in an immutable entity.");
					}
				}
			...
			foreach (var prop in typeProperties)
			{
				...
				{
					builder.AppendLineInvariant(
						$"#error {nameof(TemporaryImmutableGenerator)}: Non-static property {symbolNames.SymbolNameWithGenerics}.{prop.Name} "
			...
			foreach (var field in typeSymbol.GetFields())
			{
				...
				{
					builder.AppendLineInvariant(
						$"#error {nameof(TemporaryImmutableGenerator)}: Immutable type {symbolNames.SymbolNameWithGenerics} cannot "
